...
| CC9 | CC9.2 | CC9-05 | GitHub Actions are pinned to full SHA to prevent supply chain attacks through tag mutation. Automated dependency updates (Renovate) are designed but not yet operational — the Renovate App is not installed (config present in `renovate.json`; 0 PR history). | Preventive | DevOps Lead | Pinned workflows, `scripts/check-action-pins.sh`, `renovate.json` configuration | Continuous | Implemented (SHA-pinning); Renovate automated-update portion Planned — Renovate App not yet installed |
